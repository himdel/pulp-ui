...
describe('Namespace Access tab', () => {
  ...
  beforeEach(() => {
    ...
    cy.get('.pf-v6-c-tabs__item-text').contains('Access').click();
  });
...
function testAccessTab({
  ...
  cy.get('button').contains('Select a group').click();
  cy.get('.pf-v6-c-wizard__footer-cancel').click();
  ...
  cy.get('.pf-v6-c-table__check input[type=checkbox]')
    .not('[disabled]')
    .first()
    .click();
  cy.get('footer button').contains('Next').click();
  ...
  cy.get(
    `[data-cy="RoleListTable-ExpandableRow-row-${role}"] .pf-v6-c-table__toggle button`,
  ).click();
  cy.contains('.pf-v6-c-label', permissionLabel, { timeout: 10000 });
  ...
  cy.get('.pf-v6-c-modal-box__body b').contains('access_group');
  cy.get('.pf-v6-c-modal-box__body b').contains(role);
  cy.get('.pf-v6-c-modal-box__body b').contains(`rbac_access_${num}`);
  ...
  cy.contains('.pf-v6-c-tabs__item', 'Access').click();
  ...
  cy.get('.pf-v6-c-dropdown__menu-item').contains('Remove group').click();
  cy.get('.pf-v6-c-modal-box__body b').contains('access_group');
  cy.get('.pf-v6-c-modal-box__body b').contains(`rbac_access_${num}`);
  cy.get('[data-cy=delete-button]').click();
  cy.get('.pf-v6-c-alert__title')
